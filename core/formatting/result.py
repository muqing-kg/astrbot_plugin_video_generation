...
def format_task_detail(record: VideoTaskRecord) -> str:
    lines = [
        ...
        f"秒数: {record.duration}s",
        f"比例: {record.aspect_ratio}",
        f"分辨率: {record.resolution}",
...
def format_task_list(records: list[VideoTaskRecord]) -> str:
    ...
    for index, record in enumerate(records, start=1):
        lines.append(
            f"{index}. [{record.status_label}] {record.task_id} | {record.duration}s {record.aspect_ratio} | {record.prompt[:40]}"
        )
    return "\n".join(lines)
...
def format_result_info(record: VideoTaskRecord, items: list[str]) -> str:
    ...
    mapping = {
        ...
        "任务ID": record.task_id,
        "秒数": f"{record.duration}s",
        "比例": record.aspect_ratio,
